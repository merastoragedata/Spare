...
var SHEETS = {
  ...
  ITEMS:       ['id', 'categoryId', 'name', 'unit', 'location', 'trackLow', 'lowCutoff', 'unitValue', 'docMode', 'fields', 'remarks', 'docs', 'createdAt', 'updatedAt'],
  ...
  KITS:        ['id', 'categoryId', 'name', 'ownership', 'ownerDetail', 'status', 'location', 'returnRequired', 'fields', 'remarks', 'docs', 'createdAt', 'updatedAt'],
...
var ADMIN_FNS = ['deleteItem', 'deleteCategory', 'deleteRecord', 'deleteKit', 'deleteKitEntry', 'deleteReport',
...
function getBootstrap() {
  ...
  var items = tbl_('ITEMS').rows.map(function (i) {
    return {
      ...
      trackLow: boolv_(i.trackLow), lowCutoff: Number(i.lowCutoff || 0), unitValue: Number(i.unitValue || 0), docMode: i.docMode || 'consignment',
  ...
  var kits = tbl_('KITS').rows.map(function (k) {
    return {
      ...
      status: k.status, location: k.location, returnRequired: k.returnRequired === 'Yes', fields: json_(k.fields, {}), remarks: k.remarks, docs: json_(k.docs, []),
...
function apiAddItem(p) {
  return lock_(function () {
    ...
    insert_('ITEMS', { id: id, categoryId: p.categoryId, name: p.name, unit: p.unit || 'Nos.', location: p.location || '',
      trackLow: !!p.trackLow, lowCutoff: Number(p.lowCutoff || 0), unitValue: Number(p.unitValue || 0), docMode: p.docMode || 'consignment',
...
function apiAddKit(p) {
  return lock_(function () {
    ...
    insert_('KITS', { id: uid_('kit'), categoryId: p.categoryId, name: p.name, ownership: p.ownership || '',
      ...
      returnRequired: p.returnRequired ? 'Yes' : '',
      fields: p.fields || {}, remarks: p.remarks || '', docs: p.docs || [], createdAt: now_(), updatedAt: now_() });
    return getBootstrap();
  });
}
