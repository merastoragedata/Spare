...
var SHEETS = {
  ...
  REPORTS:     ['id', 'kind', 'title', 'desc', 'date', 'fileId', 'fileName', 'createdAt', 'updatedAt']
};
...
var ANY_ROLE_FNS = ['getBootstrap', 'getDocFull', 'getReportHtml', 'changePassword', 'whoAmI'];
...
function getBootstrap() {
  ...
  var reports = tbl_('REPORTS').rows.map(function (r) {
    return { id: r.id, kind: r.kind, title: r.title, desc: r.desc, date: dstr_(r.date), fileId: r.fileId,
      ...
      url: 'https://drive.google.com/file/d/' + r.fileId + '/view' };
...
function apiUploadReport(p) {
  return lock_(function () {
    ...
    if (!p.full) throw new Error('No file content received');
    var blob = Utilities.newBlob(Utilities.base64Decode(p.full), p.mime || 'text/html', p.name || ('report_' + Date.now() + '.html'));
    var file = getFolder_().createFile(blob);
    var id = uid_('rpt');
    insert_('REPORTS', {
      id: id, kind: p.kind, title: p.title, desc: p.desc || '', date: p.date || now_().slice(0, 10),
      fileId: file.getId(), fileName: p.name || file.getName(), createdAt: now_(), updatedAt: now_()
    });
    return getBootstrap();
  });
}
...
function apiDeleteReport(id) {
  return lock_(function () {
    var r = find_('REPORTS', id);
    if (r) { try { DriveApp.getFileById(r.fileId).setTrashed(true); } catch (e) {} deleteRow_('REPORTS', id); }
    return getBootstrap();
  });
}
