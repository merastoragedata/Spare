...
var DOCS_FOLDER_ID = '13ujwoGTO-SOTnMOhgepCEUy_pPX9VESB';
...
var __tblCache = {}; // per-request cache — reset in guarded_() at the start of every call, invalidated per-table on any write
...
var SHEETS = {
  ...
  ITEMS:       ['id', 'categoryId', 'name', 'unit', 'location', 'trackLow', 'lowCutoff', 'unitValue', 'routineTest', 'resultSheetId', 'docMode', 'sortOrder', 'fields', 'remarks', 'docs', 'createdAt', 'updatedAt'],
  ...
  DOCS:        ['id', 'title', 'fileId', 'thumb', 'uploadedBy', 'createdAt'],
  KITS:        ['id', 'categoryId', 'name', 'ownership', 'ownerDetail', 'status', 'location', 'returnRequired', 'opSheetId', 'sortOrder', 'fields', 'remarks', 'docs', 'createdAt', 'updatedAt'],
  KIT_ISSUES:  ['id', 'kitId', 'date', 'issue', 'severity', 'resolved', 'resolvedDate', 'remarks', 'docs', 'createdAt', 'updatedAt'],
  ...
  REPORTS:     ['id', 'kind', 'title', 'desc', 'date', 'fileId', 'fileName', 'xlsxId', 'xlsxName', 'pdfId', 'pdfName', 'createdAt', 'updatedAt']
};
...
var ADMIN_FNS = ['deleteItem', 'deleteCategory', 'deleteRecord', 'deleteKit', 'deleteKitEntry', 'deleteKitIssue', 'deleteReport', 'deleteDoc',
                  'listUsers', 'approveUser', 'rejectUser', 'setUserRole', 'resetPassword'];
var ANY_ROLE_FNS = ['getBootstrap', 'getDocFull', 'getReportHtml', 'getReportFile', 'changePassword', 'whoAmI'];
...
var API = {
  ...
  reorder: function (kind, ids) { return apiReorder(kind, ids); },
  ...
  uploadReport: function (p) { return apiUploadReport(p); },
  getReportHtml: function (id) { return apiGetReportHtml(id); },
  getReportFile: function (id, which) { return apiGetReportFile(id, which); },
  updateReport: function (id, patch) { return apiUpdateReport(id, patch); },
  deleteReport: function (id) { return apiDeleteReport(id); },
  ...
  ensureItemSheet: function (id) { return apiEnsureItemSheet(id); },
  ensureKitSheet: function (id) { return apiEnsureKitSheet(id); },
  addKitIssue: function (kitId, p) { return apiAddKitIssue(kitId, p); },
  updateKitIssue: function (id, p) { return apiUpdateKitIssue(id, p); },
  deleteKitIssue: function (id) { return apiDeleteKitIssue(id); },
...
function guarded_(fn, args, token) {
  __tblCache = {}; // fresh slate for this request — never trust cache leftovers from a prior execution
...
function apiChangePassword(oldPw, newPw, sess) {
  ...
  if (!newPw) throw new Error('Enter a new password.');
...
// Create a Google Sheet for a record, seed it with a heading + template, move to the app folder,
// make it link-viewable (so it can be embedded), and return its id.
function createRecordSheet_(title, kind) {
  var ss = SpreadsheetApp.create(title);
  var sh = ss.getSheets()[0];
  if (kind === 'kitOp') {
    sh.setName('Operational Info');
    sh.getRange('A1').setValue(title).setFontSize(13).setFontWeight('bold');
    sh.getRange('A2').setValue('Operational information, settings, connection notes, calibration references, do/don\'t.');
    sh.getRange('A4:C4').setValues([['Topic', 'Detail', 'Updated']]).setFontWeight('bold').setBackground('#305496').setFontColor('#ffffff');
    sh.setColumnWidth(1, 180); sh.setColumnWidth(2, 420); sh.setColumnWidth(3, 120);
    sh.setFrozenRows(4);
  } else {
    sh.setName('Test Results');
    sh.getRange('A1').setValue(title).setFontSize(13).setFontWeight('bold');
    sh.getRange('A2').setValue('Routine test results. Add columns/tabs as needed; all formatting is shown in the portal.');
    sh.getRange('A4:E4').setValues([['Date', 'Test', 'Reading', 'Result', 'Tested By']]).setFontWeight('bold').setBackground('#305496').setFontColor('#ffffff');
    sh.setColumnWidths(1, 5, 120); sh.setColumnWidth(2, 200);
    sh.setFrozenRows(4);
  }
  var file = DriveApp.getFileById(ss.getId());
  try { getFolder_().addFile(file); DriveApp.getRootFolder().removeFile(file); } catch (e) {}
  try { file.setSharing(DriveApp.Access.ANYONE_WITH_LINK, DriveApp.Permission.VIEW); } catch (e) {}
  return ss.getId();
}
function apiEnsureItemSheet(id) {
  return lock_(function () {
    var it = find_('ITEMS', id);
    if (!it) throw new Error('Material not found');
    if (!it.resultSheetId) {
      var sid = createRecordSheet_('Test Results — ' + it.name, 'itemTest');
      updateRow_('ITEMS', id, { resultSheetId: sid, updatedAt: now_() });
    }
    return getBootstrap();
  });
}
function apiEnsureKitSheet(id) {
  return lock_(function () {
    var k = find_('KITS', id);
    if (!k) throw new Error('Kit not found');
    if (!k.opSheetId) {
      var sid = createRecordSheet_('Operational Info — ' + k.name, 'kitOp');
      updateRow_('KITS', id, { opSheetId: sid, updatedAt: now_() });
    }
    return getBootstrap();
  });
}
...
function tbl_(name) {
  if (__tblCache[name]) return __tblCache[name];
  ...
  var t = { sh: sh, hm: hm, rows: rows, idField: idField };
  __tblCache[name] = t;
  return t;
}
function cell_(v) {
...
function deleteRow_(name, id) {
  ...
  for (var i = t.rows.length - 1; i >= 0; i--) {
    if (String(t.rows[i][t.idField]) === String(id)) { t.sh.deleteRow(t.rows[i].__row); delete __tblCache[name]; return true; }
  }
  return false;
}
...
function getBootstrap() {
  ...
  var items = tbl_('ITEMS').rows.map(function (i) {
    return {
      id: i.id, categoryId: i.categoryId, name: i.name, unit: i.unit, location: i.location, sortOrder: Number(i.sortOrder || 0),
      trackLow: boolv_(i.trackLow), lowCutoff: Number(i.lowCutoff || 0), unitValue: Number(i.unitValue || 0), routineTest: boolv_(i.routineTest), resultSheetId: i.resultSheetId || '', docMode: i.docMode || 'consignment',
  ...
  var docs = tbl_('DOCS').rows.map(function (d) {
    return { id: d.id, title: d.title, fileId: d.fileId, thumb: d.thumb, uploadedBy: d.uploadedBy || '',
      url: 'https://drive.google.com/file/d/' + d.fileId + '/view' };
  });

  var kmoves = tbl_('KIT_MOVES').rows, kreps = tbl_('KIT_REPAIRS').rows, kissues = tbl_('KIT_ISSUES').rows;
  var kits = tbl_('KITS').rows.map(function (k) {
    return {
      id: k.id, categoryId: k.categoryId, name: k.name, ownership: k.ownership, ownerDetail: k.ownerDetail, sortOrder: Number(k.sortOrder || 0),
      status: k.status, location: k.location, returnRequired: k.returnRequired === 'Yes', opSheetId: k.opSheetId || '',
      fields: json_(k.fields, {}), remarks: k.remarks, docs: json_(k.docs, []),
      ...
      issuesFaced: kissues.filter(function (x) { return x.kitId === k.id; }).map(function (x) {
        return { id: x.id, date: dstr_(x.date), issue: x.issue, severity: x.severity, resolved: x.resolved === 'Yes',
          resolvedDate: dstr_(x.resolvedDate), remarks: x.remarks, docs: json_(x.docs, []) };
      }).sort(function (a, b) { return (b.date || '').localeCompare(a.date || ''); })
    };
  });

  var reports = tbl_('REPORTS').rows.map(function (r) {
    return { id: r.id, kind: r.kind, title: r.title, desc: r.desc, date: dstr_(r.date), fileId: r.fileId,
      fileName: r.fileName, createdAt: r.createdAt,
      hasHtml: !!r.fileId, hasXlsx: !!r.xlsxId, hasPdf: !!r.pdfId,
      xlsxName: r.xlsxName || '', pdfName: r.pdfName || '',
      url: r.fileId ? ('https://drive.google.com/file/d/' + r.fileId + '/view') : '' };
  }).sort(function (a, b) { return (b.date || '').localeCompare(a.date || ''); });

  return {
    version: BACKEND_VERSION,
    settings: settings, uoms: uoms, ownerships: ownerships, kitStatuses: kitStatuses, moveReasons: moveReasons,
    categories: cats, items: items, docs: docs, kits: kits, reports: reports,
...
function apiAddItem(p) {
  return lock_(function () {
    ...
    insert_('ITEMS', { id: id, categoryId: p.categoryId, name: p.name, unit: p.unit || 'Nos.', location: p.location || '',
      trackLow: !!p.trackLow, lowCutoff: Number(p.lowCutoff || 0), unitValue: Number(p.unitValue || 0), routineTest: p.routineTest ? 'Yes' : '', resultSheetId: '', docMode: p.docMode || 'consignment',
...
function apiReorder(kind, ids) {
  return lock_(function () {
    var table = kind === 'kit' ? 'KITS' : 'ITEMS';
    if (!Array.isArray(ids)) throw new Error('Order list required');
    ids.forEach(function (id, idx) {
      var r = find_(table, id);
      if (r) updateRow_(table, id, { sortOrder: idx + 1, updatedAt: now_() });
    });
    return getBootstrap();
  });
}
function apiUpdateItem(id, p) {
  return lock_(function () {
    ...
    updateRow_('ITEMS', id, {
      ...
      unitValue: p.unitValue !== undefined ? Number(p.unitValue || 0) : Number(cur.unitValue || 0),
      routineTest: p.routineTest !== undefined ? (p.routineTest ? 'Yes' : '') : cur.routineTest,
      resultSheetId: cur.resultSheetId,
...
function apiUploadDoc(p, sess) {
  return lock_(function () {
    ...
    var id = uid_('doc');
    // A Sheets cell caps out around 50,000 characters — an oversized client thumbnail would
    // silently get corrupted on write, which is what caused the broken-image icons. Guard it.
    var thumb = (p.thumb && p.thumb.length < 42000) ? ('data:image/jpeg;base64,' + p.thumb) : driveThumb_(file);
    insert_('DOCS', { id: id, title: p.title || p.name || 'Document', fileId: file.getId(), thumb: thumb,
      uploadedBy: sess ? sess.username : '', createdAt: now_() });
...
function apiUnlinkDoc(kind, recordId, docId, sess) {
  return lock_(function () {
    var d = find_('DOCS', docId);
    if (d && sess && sess.role !== 'admin' && d.uploadedBy !== sess.username) {
      throw new Error('Only the person who uploaded this document, or an admin, can unlink it.');
    }
...
function apiDeleteDoc(docId, sess) {
  return lock_(function () {
    var d = find_('DOCS', docId);
    if (d && sess && sess.role !== 'admin' && d.uploadedBy !== sess.username) {
      throw new Error('Only the person who uploaded this document, or an admin, can delete it.');
    }
...
function apiUploadReport(p) {
  return lock_(function () {
    if (!p.kind || (p.kind !== 'receipt' && p.kind !== 'issued')) throw new Error('Report kind must be receipt or issued');
    if (!p.title) throw new Error('Title required');
    if (!p.full && !p.xlsxFull && !p.pdfFull) throw new Error('Attach at least one file (HTML, Excel, or PDF)');
    var folder = getFolder_();
    var htmlId = '', htmlName = '';
    if (p.full) {
      var hb = Utilities.newBlob(Utilities.base64Decode(p.full), p.mime || 'text/html', p.name || ('report_' + Date.now() + '.html'));
      var hf = folder.createFile(hb); htmlId = hf.getId(); htmlName = p.name || hf.getName();
    }
    var xId = '', xName = '';
    if (p.xlsxFull) {
      var xb = Utilities.newBlob(Utilities.base64Decode(p.xlsxFull), p.xlsxMime || 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', p.xlsxName || ('report_' + Date.now() + '.xlsx'));
      var xf = folder.createFile(xb); xId = xf.getId(); xName = p.xlsxName || xf.getName();
    }
    var pId = '', pName = '';
    if (p.pdfFull) {
      var pb = Utilities.newBlob(Utilities.base64Decode(p.pdfFull), 'application/pdf', p.pdfName || ('report_' + Date.now() + '.pdf'));
      var pf = folder.createFile(pb); pId = pf.getId(); pName = p.pdfName || pf.getName();
    }
    var id = uid_('rpt');
    insert_('REPORTS', {
      id: id, kind: p.kind, title: p.title, desc: p.desc || '', date: p.date || now_().slice(0, 10),
      fileId: htmlId, fileName: htmlName, xlsxId: xId, xlsxName: xName, pdfId: pId, pdfName: pName,
      createdAt: now_(), updatedAt: now_()
    });
    return getBootstrap();
  });
}
function apiGetReportFile(id, which) {
  var r = find_('REPORTS', id);
  if (!r) throw new Error('Report not found');
  var fid = which === 'xlsx' ? r.xlsxId : which === 'pdf' ? r.pdfId : r.fileId;
  if (!fid) throw new Error('No ' + (which || 'html') + ' file for this report');
  var blob = DriveApp.getFileById(fid).getBlob();
  if (which === 'html') return blob.getDataAsString('UTF-8');
  return Utilities.base64Encode(blob.getBytes()); // binary → base64 for xlsx/pdf
}
function apiGetReportHtml(id) {
  var r = find_('REPORTS', id);
  if (!r) throw new Error('Report not found');
  return DriveApp.getFileById(r.fileId).getBlob().getDataAsString('UTF-8');
}
function apiUpdateReport(id, patch) {
  return lock_(function () {
    var r = find_('REPORTS', id);
    if (!r) throw new Error('Report not found');
    var allowed = {};
    ['title', 'desc', 'date'].forEach(function (k) { if (patch[k] !== undefined) allowed[k] = patch[k]; });
    allowed.updatedAt = now_();
    updateRow_('REPORTS', id, allowed);
    return getBootstrap();
  });
}
function apiDeleteReport(id) {
  return lock_(function () {
    var r = find_('REPORTS', id);
    if (r) {
      [r.fileId, r.xlsxId, r.pdfId].forEach(function (fid) { if (fid) { try { DriveApp.getFileById(fid).setTrashed(true); } catch (e) {} } });
      deleteRow_('REPORTS', id);
    }
    return getBootstrap();
  });
}
...
function apiAddKitIssue(kitId, p) {
  return lock_(function () {
    if (!find_('KITS', kitId)) throw new Error('Kit not found');
    if (!p.issue) throw new Error('Describe the issue');
    insert_('KIT_ISSUES', { id: uid_('kiss'), kitId: kitId, date: p.date || now_().slice(0, 10), issue: p.issue,
      severity: p.severity || '', resolved: p.resolved ? 'Yes' : '', resolvedDate: p.resolvedDate || '',
      remarks: p.remarks || '', docs: p.docs || [], createdAt: now_(), updatedAt: now_() });
    return getBootstrap();
  });
}
function apiUpdateKitIssue(id, p) {
  return lock_(function () {
    var cur = find_('KIT_ISSUES', id);
    if (!cur) throw new Error('Issue not found');
    updateRow_('KIT_ISSUES', id, {
      date: p.date !== undefined ? p.date : cur.date, issue: p.issue !== undefined ? p.issue : cur.issue,
      severity: p.severity !== undefined ? p.severity : cur.severity,
      resolved: p.resolved !== undefined ? (p.resolved ? 'Yes' : '') : cur.resolved,
      resolvedDate: p.resolvedDate !== undefined ? p.resolvedDate : cur.resolvedDate,
      remarks: p.remarks !== undefined ? p.remarks : cur.remarks,
...
function apiDeleteKitIssue(id) {
  return lock_(function () { deleteRow_('KIT_ISSUES', id); return getBootstrap(); });
}
/* Run once from the editor: creates the Google Sheet for every routine-test item and every kit that lacks one. */
function ensureAllSheets() {
  var made = 0;
  tbl_('ITEMS').rows.forEach(function (it) {
    if (boolv_(it.routineTest) && !it.resultSheetId) {
      updateRow_('ITEMS', it.id, { resultSheetId: createRecordSheet_('Test Results — ' + it.name, 'itemTest'), updatedAt: now_() });
      made++;
    }
  });
  tbl_('KITS').rows.forEach(function (k) {
    if (!k.opSheetId) {
      updateRow_('KITS', k.id, { opSheetId: createRecordSheet_('Operational Info — ' + k.name, 'kitOp'), updatedAt: now_() });
      made++;
    }
  });
  var msg = 'Sheet creation complete. Created ' + made + ' new sheet(s).';
...
function apiAddKit(p) {
  return lock_(function () {
    ...
    insert_('KITS', { id: uid_('kit'), categoryId: p.categoryId, name: p.name, ownership: p.ownership || '',
      ...
      returnRequired: p.returnRequired ? 'Yes' : '', opSheetId: '',
      fields: p.fields || {}, remarks: p.remarks || '', docs: p.docs || [], createdAt: now_(), updatedAt: now_() });
    return getBootstrap();
  });
}
function apiUpdateKit(id, p) {
  return lock_(function () {
    ...
    updateRow_('KITS', id, {
      ...
      returnRequired: p.returnRequired !== undefined ? (p.returnRequired ? 'Yes' : '') : cur.returnRequired,
...
function kitLastMove_(kitId) {
  var mv = tbl_('KIT_MOVES').rows.filter(function (r) { return r.kitId === kitId; })
    .sort(function (a, b) { return (a.date || '').localeCompare(b.date || ''); });
  return mv.length ? mv[mv.length - 1] : null;
}
function kitOpenRepair_(kitId) {
  var rp = tbl_('KIT_REPAIRS').rows.filter(function (r) { return r.kitId === kitId && r.status !== 'Received'; })
    .sort(function (a, b) { return (b.dateSent || '').localeCompare(a.dateSent || ''); });
  return rp.length ? rp[0] : null;
}
function apiAddKitMove(kitId, p) {
  return lock_(function () {
    ...
    var last = kitLastMove_(kitId);
    if (p.direction === 'OUT' && kitOpenRepair_(kitId)) {
      throw new Error('This kit is currently at repair/calibration — receive it before giving it out.');
    }
    if (p.direction === 'IN' && (!last || last.direction !== 'OUT')) {
      throw new Error('This kit is not currently given out, so it cannot be taken back.');
    }
...
function apiAddKitRepair(kitId, p) {
  return lock_(function () {
    ...
    if (kitOpenRepair_(kitId)) throw new Error('This kit already has an open repair entry — receive it before sending it again.');
    var lastMove = kitLastMove_(kitId);
    if (lastMove && lastMove.direction === 'OUT') throw new Error('This kit is currently given out — take it back before sending it for repair.');
...
function seedMaterials_() {
  var existingCatIds = {}; tbl_('CATEGORIES').rows.forEach(function (c) { existingCatIds[c.id] = true; });
  [
  ...
  ].forEach(function (c) { if (!existingCatIds[c.id]) insert_('CATEGORIES', c); });

  setSettingRaw_('siteName', '400 kV Karjat Substation');
  setSettingRaw_('division', '400 kV RS Division, Karjat');
  setSettingRaw_('uoms', JSON.stringify(DEFAULT_UOMS));
  setSettingRaw_('kitOwnerships', JSON.stringify(DEFAULT_OWNERSHIPS));
  setSettingRaw_('kitStatuses', JSON.stringify(DEFAULT_KIT_STATUSES));
  setSettingRaw_('kitMoveReasons', JSON.stringify(DEFAULT_MOVE_REASONS));
  ...
  function add_(item, rc) {
    if (find_('ITEMS', item.id)) return; // already seeded — never duplicate
...
/* ======================================================================== *
 *  MANUAL SEEDER — run this ONCE from the editor if the Dhule CT/LA/SF6/
 *  NIFPS material is missing from Items. Safe to re-run: skips any
 *  category or item that already exists, matched by id, and never
 *  duplicates settings rows. Works on an existing database — no rebuild.
 *  After it runs, refresh the web app; the items appear under their
 *  categories with their receipts already logged. Run indexDocuments()
 *  afterwards (or Sync folder) to attach the nameplate/gate-pass images.
 * ======================================================================== */
function seedMaterials() {
  var before = tbl_('ITEMS').rows.length;
  seedMaterials_();
  var after = tbl_('ITEMS').rows.length;
  var msg = 'Material seed complete. Added ' + (after - before) + ' item(s); ' + before
          + ' already present. Total items now: ' + after + '.';
  Logger.log(msg);
  return msg;
}
function seedKits_() {
