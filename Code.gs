 ...
 * ============================================================================
 *  SETUP
 *   1. Run  ▶ setupDatabase()      → builds the Sheet + seed material data
 *   2. Run  ▶ indexDocuments()     → registers the images ALREADY in your
 *                                    Drive folder (no copying) and links them
 *                                    to the right material
 *   3. Deploy ▸ New deployment ▸ Web app
 *        Execute as: Me      Who has access: Anyone
 *   4. Run  ▶ healthCheck()
 *
 *  Every document — seeded or uploaded later from the portal — lives in the
 *  ONE folder below. Nothing is ever duplicated.
 * ============================================================================
 */

/* ⇩⇩⇩  YOUR DRIVE DOCUMENTS FOLDER  ⇩⇩⇩ ---------------------------------- */
var DOCS_FOLDER_ID = '1jOe2IPDvrOp7VpDVOdurni0XzG0-TtYO';
/* ------------------------------------------------------------------------ */

var CFG = { SS_NAME: 'Karjat Spare Material Register (Database)', P_SS: 'SS_ID' };

var SHEETS = {
  SETTINGS:   ['key', 'value'],
  CATEGORIES: ['id', 'name', 'desc', 'fields'],
  ITEMS:      ['id', 'categoryId', 'name', 'unit', 'location', 'trackLow', 'lowCutoff', 'fields', 'remarks', 'docs', 'createdAt', 'updatedAt'],
  ...
  ISSUES:     ['id', 'itemId', 'date', 'qty', 'purpose', 'issuedTo', 'location', 'authorisedBy', 'workDetails', 'remarks', 'docs', 'createdAt', 'updatedAt'],
...
function doGet(e) {
  ...
  try {
    ...
    return ContentService.createTextOutput(JSON.stringify({ ok: true, msg: 'Spare Material Register API is live.' }))
...
function tbl_(name) {
  ...
  var last = sh.getLastRow(), head = SHEETS[name], rows = [];
  if (last > 1) {
...
function insert_(name, obj) { var t = tbl_(name); t.sh.appendRow(t.head.map(function (k) { return cell_(obj[k]); })); return obj; }
function find_(name, id) { var hit = null; tbl_(name).rows.forEach(function (r) { if (String(r.id) === String(id)) hit = r; }); return hit; }
...
function getBootstrap() {
  ...
  var items = tbl_('ITEMS').rows.map(function (i) {
    return {
      ...
      lowCutoff: Number(i.lowCutoff || 0), fields: json_(i.fields, {}), remarks: i.remarks,
      docs: json_(i.docs, []), createdAt: i.createdAt,
      ...
      issues: isss.filter(function (r) { return r.itemId === i.id; }).map(function (r) {
        return { id: r.id, date: dstr_(r.date), qty: Number(r.qty || 0), purpose: r.purpose, issuedTo: r.issuedTo,
  ...
  return {
    settings: settings, uoms: uoms, categories: cats, items: items, docs: docs,
...
function apiAddItem(p) {
  return lock_(function () {
    ...
    var id = uid_('itm');
    insert_('ITEMS', { id: id, categoryId: p.categoryId, name: p.name, unit: p.unit || 'Nos.',
...
function apiUpdateItem(id, p) {
  return lock_(function () {
    ...
    updateRow_('ITEMS', id, {
      ...
      trackLow: !!p.trackLow, lowCutoff: Number(p.lowCutoff || 0),
...
/* ======================================================================== *
 *  RECEIPTS / ISSUES  (add · edit · delete)
 * ======================================================================== */

function apiAddReceipt(itemId, r) {
  return lock_(function () {
    if (!(Number(r.qty) > 0)) throw new Error('Quantity must be greater than zero');
    insert_('RECEIPTS', { id: uid_('rcp'), itemId: itemId, date: r.date, qty: Number(r.qty),
      source: r.source || '', refType: r.refType || '', refNo: r.refNo || '', invoiceNo: r.invoiceNo || '',
      vehicle: r.vehicle || '', receivedBy: r.receivedBy || '', unitRate: Number(r.unitRate || 0),
      taxableAmt: Number(r.taxableAmt || 0), gstPct: Number(r.gstPct || 0), totalAmt: Number(r.totalAmt || 0),
      remarks: r.remarks || '', docs: r.docs || [], createdAt: now_(), updatedAt: now_() });
    return getBootstrap();
  });
}
...
function apiUpdateReceipt(id, r) {
  return lock_(function () {
    ...
    // editing down must not make the balance negative
    var recd = 0, issd = 0;
    ...
    updateRow_('RECEIPTS', id, {
      date: r.date, qty: q, source: r.source || '', refType: r.refType || '', refNo: r.refNo || '',
      invoiceNo: r.invoiceNo || '', vehicle: r.vehicle || '', receivedBy: r.receivedBy || '',
      unitRate: Number(r.unitRate || 0), taxableAmt: Number(r.taxableAmt || 0), gstPct: Number(r.gstPct || 0),
      totalAmt: Number(r.totalAmt || 0), remarks: r.remarks || '',
      docs: r.docs !== undefined ? r.docs : json_(cur.docs, []), updatedAt: now_()
    });
    return getBootstrap();
  });
}
...
function apiAddIssue(itemId, r) {
  return lock_(function () {
    ...
    insert_('ISSUES', { id: uid_('iss'), itemId: itemId, date: r.date, qty: q, purpose: r.purpose,
      issuedTo: r.issuedTo || '', location: r.location || '', authorisedBy: r.authorisedBy || '',
      workDetails: r.workDetails || '', remarks: r.remarks || '', docs: r.docs || [],
...
function apiUpdateIssue(id, r) {
  return lock_(function () {
    ...
    if (!cur) throw new Error('Issue record not found');
    var q = Number(r.qty);
    if (!(q > 0)) throw new Error('Quantity must be greater than zero');
    if (!r.purpose) throw new Error('Purpose of use is required');
    var recd = 0, issd = 0;
    ...
    updateRow_('ISSUES', id, { date: r.date, qty: q, purpose: r.purpose, issuedTo: r.issuedTo || '',
      location: r.location || '', authorisedBy: r.authorisedBy || '', workDetails: r.workDetails || '',
      remarks: r.remarks || '', docs: r.docs !== undefined ? r.docs : json_(cur.docs, []), updatedAt: now_() });
    return getBootstrap();
  });
}

/** Also used to REVERT an issue (the stock goes straight back to balance). */
...
function apiUpdateField(catId, key, patch) {
  return lock_(function () {
    ...
    ['label', 'type', 'required', 'hint', 'options'].forEach(function (k) {
...
function apiMoveField(catId, key, dir) {
  return lock_(function () {
    ...
    var j = i + (dir === 'up' ? -1 : 1);
    if (j < 0 || j >= f.fields.length) return getBootstrap();
    var tmp = f.fields[i]; f.fields[i] = f.fields[j]; f.fields[j] = tmp;
    updateRow_('CATEGORIES', catId, { fields: f.fields });
    return getBootstrap();
  });
}
/** "Other…" typed by an editor → promoted to a permanent dropdown option. */
...
/* ======================================================================== *
 *  UNIT OF MEASURE  (learned)
 * ======================================================================== */

function apiAddUom(u) {
  return lock_(function () {
    u = String(u || '').trim();
    if (!u) throw new Error('Empty unit');
    var t = tbl_('SETTINGS'), hit = null;
    t.rows.forEach(function (r) { if (r.key === 'uoms') hit = r; });
    var list = hit ? json_(hit.value, DEFAULT_UOMS) : DEFAULT_UOMS.slice();
    if (list.indexOf(u) < 0) list.push(u);
    if (hit) t.sh.getRange(hit.__row, 2).setValue(JSON.stringify(list));
    else t.sh.appendRow(['uoms', JSON.stringify(list)]);
    return getBootstrap();
  });
}
...
/* ======================================================================== *
 *  DOCUMENTS — one folder, no duplication
 * ======================================================================== */
...
function apiUploadDoc(p) {
  return lock_(function () {
    ...
    var file = getFolder_().createFile(blob);           // straight into THE folder
    var id = uid_('doc');
...
function sheetFor_(kind) { return kind === 'item' ? 'ITEMS' : (kind === 'receipt' ? 'RECEIPTS' : 'ISSUES'); }
...
function apiLinkDocs(kind, recordId, docIds) {
  return lock_(function () { apiLinkDocs_(kind, recordId, docIds); return getBootstrap(); });
}
...
/** Picks up anything new dropped straight into the Drive folder. */
function apiSyncFolder() {
  return lock_(function () {
    ...
    while (it.hasNext()) {
      ...
      insert_('DOCS', { id: uid_('doc'), title: prettyName_(f.getName()), fileId: f.getId(),
                        thumb: driveThumb_(f), createdAt: now_() });
      added++;
    }
    return getBootstrap();
  });
}
function prettyName_(n) {
  return String(n).replace(/\.[a-z0-9]+$/i, '').replace(/^\d+[_\-\s]*/, '').replace(/[_\-]+/g, ' ').replace(/\s+/g, ' ').trim();
}

/* ======================================================================== *
 *  DOCUMENT INDEXING — registers files already in the folder (no copying)
 * ======================================================================== */
...
/** STEP 2 — register the images that are already sitting in your Drive folder. */
function indexDocuments() {
  ...
  SEED_DOCS.forEach(function (d) {
    try {
      ...
      var it = folder.getFilesByName(d.file);
      if (!it.hasNext()) { log.push('✗ NOT FOUND in folder: ' + d.file); return; }
      var file = it.next();
      var id = byFile[file.getId()] || uid_('doc');
      if (!byFile[file.getId()]) {
        insert_('DOCS', { id: id, title: d.title, fileId: file.getId(), thumb: driveThumb_(file), createdAt: now_() });
      }
      d.items.forEach(function (itemId) {
        if (!find_('ITEMS', itemId)) { log.push('  (item missing: ' + itemId + ')'); return; }
      ...
      log.push('✓ ' + d.file + '  →  ' + d.items.join(', '));
    } catch (err) { log.push('✗ ' + d.file + ' — ' + err.message); }
  });

  var extra = apiSyncFolder();
  log.push('');
  log.push('Folder now has ' + extra.docs.length + ' registered document(s).');
  Logger.log(log.join('\n'));
  return log.join('\n');
}
...
function setupDatabase() {
  ...
  if (old) {
    try { SpreadsheetApp.openById(old); throw new Error('Database already exists — run rebuildEverything() to start over, or openDatabase() to see it.'); }
    catch (e) { if (String(e.message).indexOf('already exists') > -1) throw e; }
  }
...
function reloadSeedData() {
  ...
  seed_();
  return 'Seed material reloaded. Run indexDocuments() again to relink documents.';
}
...
function healthCheck() {
  ...
  try {
    ...
    Object.keys(SHEETS).forEach(function (n) { out.push('   ' + n + ': ' + Math.max(0, ss.getSheetByName(n).getLastRow() - 1) + ' row(s)'); });
  } catch (e) { out.push('Sheet  FAIL: ' + e.message); }
  try {
    var f = getFolder_(); var n = 0, it = f.getFiles(); while (it.hasNext()) { it.next(); n++; }
    out.push('Folder OK : ' + f.getName() + ' (' + n + ' file(s)) — https://drive.google.com/drive/folders/' + DOCS_FOLDER_ID);
  } catch (e) { out.push('Folder FAIL: ' + e.message); }
  try {
    var b = getBootstrap();
    out.push('API    OK : ' + b.items.length + ' items · ' + b.categories.length + ' categories · ' + b.docs.length + ' documents · ' + b.uoms.length + ' UoMs');
    var linked = 0; b.items.forEach(function (i) { linked += (i.docs || []).length; });
    out.push('Documents linked to materials: ' + linked);
    out.push('Low-stock flagged: ' + b.items.filter(function (i) { return i.trackLow; }).length + ' of ' + b.items.length);
...
function seed_() {
  [
    { id: 'cat_ehv', name: 'EHV Equipment', desc: 'Circuit breakers, CTs, CVTs, PTs, isolators, isolator blades, discharge rods and other EHV switchyard equipment.',
      fields: [
        { key: 'equipType', label: 'Equipment Type', type: 'select', required: true, hint: 'What kind of switchyard equipment this is', options: ['Circuit Breaker', 'Current Transformer', 'CVT', 'Potential Transformer', 'Surge Arrester', 'Isolator', 'Isolator Blade', 'Discharge Rod', 'Wave Trap'] },
        { key: 'voltageClass', label: 'Voltage Class', type: 'select', required: true, hint: 'Highest system voltage', options: ['11 kV', '33 kV', '132 kV', '198 kV', '220 kV', '245 kV', '390 kV', '400 kV', '420 kV', '765 kV'] },
        { key: 'make', label: 'Make / Manufacturer', type: 'text', required: true, hint: 'e.g. MEHRU, LAMCO, CGL' },
        { key: 'typeModel', label: 'Type / Model', type: 'text', hint: 'Manufacturer type designation' },
  ...
  var CTR = { source: 'CTR Manufacturing Industries Pvt. Ltd., Nagar Road, Pune - 411014',
              refType: 'Invoice & Delivery Challan', refNo: 'LR B4002899760 dtd. 13.07.2026 (Associated Road Carriers Ltd.)',
              invoiceNo: '2GSSIA1434 dtd. 13.07.2026', vehicle: 'Door delivery — ARC, ex-Wagholi (WGLB)', receivedBy: '' };

  function add_(item, rc) {
    insert_('ITEMS', { id: item.id, categoryId: item.categoryId, name: item.name, unit: item.unit,
      location: item.location, trackLow: item.trackLow, lowCutoff: item.lowCutoff, fields: item.fields,
      remarks: item.remarks, docs: [], createdAt: item.createdAt, updatedAt: item.createdAt });
    insert_('RECEIPTS', { id: uid_('rcp'), itemId: item.id, date: rc.date, qty: rc.qty,
      source: rc.base.source, refType: rc.base.refType, refNo: rc.base.refNo, invoiceNo: rc.base.invoiceNo,
      vehicle: rc.base.vehicle, receivedBy: rc.base.receivedBy, unitRate: rc.unitRate || 0,
      taxableAmt: rc.taxableAmt || 0, gstPct: rc.gstPct || 0, totalAmt: rc.totalAmt || 0,
  ...
  add_({ id: 'itm_la390', categoryId: 'cat_ehv', name: '390 kV, 20 kA MO Gapless Surge Arrester (LAMCO)', unit: 'Set',
    ...
    fields: { equipType: 'Surge Arrester', voltageClass: '390 kV', make: 'LAMCO Industries Pvt. Ltd.',
      ...
      serialNos: '40 A\n40 B\n40 C\n40 D', yearMfg: 2025, poLoa: 'SP/L-14/T-0622/0724/LAMCO/NO.00714 dtd. 23.05.2025' },
    remarks: '1 Set = 4 series stacks (40 A/B/C/D) with all accessories. Pressure relief current 40 kA, 50 Hz. Allotted 3 Nos.; only 1 lifted — balance 2 Nos. available at 400 kV BSL-2 Dn.',
    createdAt: '2026-07-06T10:00:00.000Z' },
    { base: GP, date: '2026-07-06', qty: 1, remarks: 'Allotted 3 Nos.; 1 Set received. Balance 2 Nos. to be lifted from 400 kV BSL-2 Dn.' });
